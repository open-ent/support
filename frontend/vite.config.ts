  ...
  return defineConfig({
    ...
    server: {
      ...
      proxy: {
        ...
        '^/(?=auth|appregistry|cas|userbook|directory|communication|conversation|portal|session|timeline|workspace|infra|support)':
          proxyObj,
        '/explorer': proxyObj,
      },
      port: 4200,
    ...
    test: {
      ...
      reporters: ['default'],
      coverage: {
        reportsDirectory: './coverage/support',
        provider: 'v8',
      },
      server: {
        deps: {
